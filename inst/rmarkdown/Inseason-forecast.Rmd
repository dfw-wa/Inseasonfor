...
for (season_i in c("spring","summer","fall")){#
  ...
  if (forecastdate <
   ...
   #change season start and end
   lubridate::year(seas_start)<-(forecast_year-1)
   lubridate::year(seas_end)<-(forecast_year-1)
...
Fall_split_cnts <- retry_get_data(
  quote(tule_bright_split(forecastdate_i)),
  name = "Fall split counts"
      ...
      render_tab(pred_date=forecastdate_i,
                 ...
                 write_local=params$use_dev_version,
                 morph="",
                 ref_years = if (season_i == "summer") c(2015) else NULL)
...
sk_seas_start <- as.Date(paste0(forecast_year, "-06-15"))
sk_seas_end   <- as.Date(paste0(forecast_year, "-07-31"))

if (forecastdate > sk_seas_end) {
  forecastdate_sk <- sk_seas_end
} else {
  forecastdate_sk <- forecastdate
}

sk_current_yr_count <- Bon_cnts |>
  dplyr::filter(lubridate::year(CountDate) == forecast_year,
                CountDate <= forecastdate) |>
  dplyr::summarise(total = sum(Sockeye, na.rm = TRUE)) |>
  dplyr::pull(total)

if (forecastdate < sk_seas_start & sk_current_yr_count < 100) {
  cat(sprintf("**The %s sockeye updates will start when counts exceed 100 fish. Below are results from %s.**",
              forecast_year, forecast_year - 1), "\n\n")
  forecastdate_sk <- as.Date(paste0(forecast_year - 1, "-07-31"))
}

render_tab_sk(pred_date   = forecastdate_sk,
              counts      = Bon_cnts,
              river_env   = flow_temp_dat,
              write_local = params$use_dev_version)
...
**Code** used to generate predictions and render website is available [here](https://github.com/dfw-wa/Inseasonfor)
